...
if (-not (Test-Path -LiteralPath $homePath)) {
    throw "Cannot find components\IntegratedHomePage.tsx. Run this script from the project root."
}

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backupRoot = Join-Path $root "_backups\intern-hiring-slider-$timestamp"
...
Write-Utf8 (Join-Path $root "components\InternHiringSlider.tsx") $component
Write-Utf8 (Join-Path $root "app\careers\internship\page.tsx") $detailsPage
Backup-File $homePath

$homeContent = [System.IO.File]::ReadAllText($homePath, [System.Text.Encoding]::UTF8)
$importLine = 'import InternHiringSlider from "@/components/InternHiringSlider";'
$anchorImport = 'import WorkshopNoticeSlider from "@/components/WorkshopNoticeSlider";'

if (-not $homeContent.Contains($importLine)) {
    if (-not $homeContent.Contains($anchorImport)) {
        throw "Could not find WorkshopNoticeSlider import in IntegratedHomePage.tsx."
    }
    $homeContent = $homeContent.Replace($anchorImport, $anchorImport + "`r`n" + $importLine)
}

if (-not $homeContent.Contains("<InternHiringSlider />")) {
    $sectionAnchor = '<WorkshopNoticeSlider notices={workshopNotices} />'
    if (-not $homeContent.Contains($sectionAnchor)) {
        throw "Could not find WorkshopNoticeSlider usage in IntegratedHomePage.tsx."
    }
    $homeContent = $homeContent.Replace($sectionAnchor, $sectionAnchor + "`r`n      <InternHiringSlider />")
}
...
Write-Host ""
Write-Host "Intern hiring slider installed." -ForegroundColor Green
...
Write-Host ""
Write-Host "INTERN HIRING SLIDER COMPLETE" -ForegroundColor Green
Write-Host "Homepage: /#internships" -ForegroundColor Cyan
Write-Host "Details: /careers/internship" -ForegroundColor Cyan
Write-Host "Run: npm.cmd run dev" -ForegroundColor Yellow
