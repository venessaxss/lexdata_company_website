...
if ($homePageContent -notmatch 'DashboardBoardSlider') {
...
if ($homePageContent.Contains($oldStage)) {
    $homePageContent = $homePageContent.Replace($oldStage, $newStage)
} elseif ($homePageContent -notmatch 'lx-dashboard-stage-sliding') {
    ...
    $homePageContent = [regex]::Replace($homePageContent, $pattern, $replacement, 1)
}
