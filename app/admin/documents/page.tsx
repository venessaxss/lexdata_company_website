...
import { approveCertificateAction, attachAuthorityReferenceAction, revokeDocumentAction, updateIssuerProfileAction } from "./actions";
...
export default async function AdminDocumentsPage({ searchParams }: { searchParams: Promise<{ message?: string; error?: string }> }) {
  ...
  const [{ data: documents, error }, { data: issuerProfiles }, { data: auditRows }] = await Promise.all([
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-10">
      <section className="mx-auto max-w-7xl">
        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-3">
          {[["Pending certificate review", pending], ["Currently issued", issued], ["Revoked / void", voided]].map(([label, count]) => (
