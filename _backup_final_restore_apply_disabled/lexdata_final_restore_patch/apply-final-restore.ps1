...
function Copy-File($relative) {
  ...
  if (!(Test-Path $src)) { throw "Missing patch file: $relative" }
...
foreach ($p in $paths) { Copy-File $p }

# Ensure homepage CSS is imported once.
...
if (Test-Path $globalsPath) {
  ...
  if ($globals -notmatch '@import "\.\/ellipsus-home\.css";' -and $globals -notmatch '@import "\.\/ellipsus-home\.css"') {
...
# Mount AuthSync in the existing layout without changing the navbar.
$layoutPath = Join-Path $root "app\layout.tsx"
if (Test-Path $layoutPath) {
  Backup-Path "app\layout.tsx"
  $layout = [System.IO.File]::ReadAllText($layoutPath)

  # Fix the common broken metadata template if a previous patch damaged it.
  ...
  if ($layout -notmatch 'import AuthSync from "@/components/AuthSync";') {
    ...
    if ($firstImport.Success) {
      $layout = $layout.Insert($firstImport.Index + $firstImport.Length, "`r`nimport AuthSync from ""@/components/AuthSync"";")
    } else {
      $layout = 'import AuthSync from "@/components/AuthSync";' + "`r`n" + $layout
    }
  }

  # Remove duplicate existing placements, then add once before body close.
...
# Disable old middleware so proxy.ts is the only session gate.
...
# Clean direct login redirects inside protected sections; proxy handles login.
foreach ($dir in @("app\dashboard", "app\admin", "app\manager", "app\my", "app\speaker")) {
  ...
  Get-ChildItem $fullDir -Recurse -Include *.ts,*.tsx | ForEach-Object {
